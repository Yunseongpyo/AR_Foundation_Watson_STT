...
namespace IBM.Watsson.Examples
{
    ...
    {
        public static ExampleStreaming instance = null;
        ...
        void Start()
        {
            instance = this;
            text = "---------";
            //Android Mike Permission(매뉴얼에는 Microphone으로 자동으로 권한 허용 메세지가 뜬다고 하지만 권한이 안떠서 강제로 물어보게 함)
        ...
        private IEnumerator CreateService()
        {
            ...
            //처음 시작할때 비활성화를 위한 false, 버튼이벤트로 Active 활성화
            Active = false;
            StartRecording();
        }
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        text = string.Format("{0}", alt.transcript, res.final ? "Final" : "Interim", alt.confidence);
